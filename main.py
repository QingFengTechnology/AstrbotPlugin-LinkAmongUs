...
from .function.task import verification_timeout_checker, group_verification_timeout_checker
...
class LinkAmongUs(Star):
    def __init__(self, context: Context, config: AstrBotConfig): # AstrBotConfig 继承自 Dict，拥有字典的所有方法。
        ...
        # 生成帮助菜单
        metadata = context.get_registered_star("astrbot_plugin_link_amongus")
        self.help_menu = help_menu(metadata)
    ...
    async def initialize(self):
        ...
        if self.KickNewMemberConfig_KickNewMemberIfNotVerify != 0:
            asyncio.create_task(group_verification_timeout_checker(self.running, self.db_pool, self.KickNewMemberConfig_PollingInterval))
    ...
    async def verify_create(self, event: AstrMessageEvent, friend_code: str):
        ...
        if active_verify_request:
            ...
            target_friend_code = active_verify_request["UserFriendCode"]
            if status in ["Created", "Retrying"]:
                logger.info(f"[LinkAmongUs] 用户 {user_qq_id} 已有进行中的验证请求，拒绝重复创建验证请求。")
                yield event.plain_result(f"你已于 {create_time} 使用 {target_friend_code} 创建了一个验证请求，需要加入服务器 {self.APIConfig_ServerName} 房间 {verify_code} 以完成验证。\n请先完成或取消该请求。")
                return
        ...
        target_is_group = group_id is not None
    ...
    async def group_ban_lift_ban(self, event: AstrMessageEvent):
        ...
        get_result = await database_manage(self.db_pool, "VerifyGroupLog", "get", user_qq_id=banned_user_id, group_id=group_id, status="Banned")
        if not get_result["success"]:
            return
        if not get_result["data"]:
